...
var matchmakeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		Ctx.Printf(":magnifying_glass_tilted_left:Attempting to find a match for module %d with server version %s...\n", matchRequest.ModuleID, matchRequest.ServerVersion)
		...
		Ctx.Println(":video_game:Match found! Gameserver address: ", addr.String())
...
func gameserverReadLoop(addr *net.UDPAddr) {
	Ctx.Println(":satellite:Connecting to gameserver at ", addr.String())
	if err := Ctx.MatchmakingClient.DialGameserver(addr); err != nil {
		Ctx.Println(":warning:Could not connect to gameserver at ", addr.String())
		return
	}
	...
	for {
		...
		if err = Ctx.MatchmakingClient.SendMessageToGameserver([]byte(answer)); err != nil {
			Ctx.Println(":warning:Could not send message to gameserver: ", err)
			continue
		}

		res, err := Ctx.MatchmakingClient.ReadMessageFromGameserver()
		if err != nil {
			Ctx.Println(":warning:Could not read message from gameserver: ", err)
			continue
		}

		Ctx.Println(":arrow_right:Response: ", string(res))
	}

	Ctx.Println("\n:stop_sign:Closing connection to gameserver at ", addr.String())
	if err := Ctx.MatchmakingClient.CloseGameserverConnection(); err != nil {
		Ctx.Println(":warning:Could not close connection to gameserver at ", addr.String())
	}
